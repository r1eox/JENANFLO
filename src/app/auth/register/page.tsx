...
import { useToast } from "../../_components/ToastProvider";
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(false);
    if (!res.ok) {
      toast.showToast(data.error || "حدث خطأ أثناء إنشاء الحساب", "error");
    } else {
      toast.showToast(data.message || "تم إنشاء الحساب بنجاح", "success");
      setName("");
      setEmail("");
      setPassword("");
    }
  ...
  return (
    <main className="min-h-screen flex items-center justify-center bg-gradient-to-br from-pink-50 to-white">
      <form className="bg-white shadow-lg rounded-xl p-8 w-full max-w-md flex flex-col gap-6" onSubmit={handleSubmit}>
        <h1 className="text-2xl font-bold text-pink-700 text-center mb-4">إنشاء حساب جديد</h1>
        <input type="text" placeholder="الاسم" className="border rounded p-3" value={name} onChange={e => setName(e.target.value)} required />
        ...
        <button type="submit" className="bg-pink-600 text-white py-3 rounded font-bold hover:bg-pink-700 transition" disabled={loading}>{loading ? "...جاري التسجيل" : "تسجيل"}</button>
      </form>
    </main>
  );
}
